...
def minimax(board):
    ...
    #         for x in actions(board):
    #             print(x)
    #             if (utility(result(board,x)) == 1):
    #                 return x
    #             else:
    #                 minimax(result(board,x))
    #     if turn == O:
    #         for x in actions(board):
    #             if (utility(result(board,x)) == -1):
    #                 return x
    ...
    #
    #     list = []
    #     if (terminal(test_board)==False):
    #         return utility(test_board)
    #     for x in actions(test_board):
    #         # if (minFunction(result(test_board, x)) > value):
    #         #     value = minFunction(result(test_board, x))
    #         #     move = x
    #         # (min_value, min_i, min_j) = minFunction(test_board)
    #         node = (minFunction(test_board))[0]
    #         if node > value:
    #             value = node
    #             i = x[0]
    #             j = x[1]
    ...
    #         return utility(test_board)
    #     for x in actions(test_board):
    #         # if (maxFunction(result(test_board, x)) < value):
    #         #     value = maxFunction(result(test_board, x))
    #         #     move = x
    #         # (max_value, max_i, max_j) = minFunction(test_board)
    #         node = (maxFunction(test_board))[0]
    #         if node > value:
    #             value = node
    #             i = x[0]
    #             j = x[1]
    ...
    # if player(board) == X:
    #     for x in (1,2):
    #         move.append(maxFunction(board))
    # else:
    #     for x in (1, 2):
    #         move.append(minFunction(board))
    # return move
    raise NotImplementedError
